...
2년간 LIMS/QMS 엔터프라이즈 시스템을 운영하며 복잡한 비즈니스 로직을 안정적으로 관리해왔고, 개인 프로젝트를 통해 Spring Boot, JPA, AWS 등 현대적인 기술 스택을 학습하였습니다.
DDD 기반 설계와 클라우드 인프라 구축 경험을 활용하여 확장 가능한 백엔드 시스템 개발에 기여하고자 합니다.
...
> LIMS(실험실 정보 관리 시스템), QMS(품질 관리 시스템) 운영 및 유지보수 참여   
> Java, Spring Framework, MyBatis, Oracle
...
**주요 성과 및 구현 내용**
- **도메인 설계**: [DDD 기반](https://9osari.netlify.app/posts/%EB%8F%84%EB%A9%94%EC%9D%B8%EC%A3%BC%EB%8F%84%EC%84%A4%EA%B3%84/) Aggregate Root 설계, 도메인 간 책임 분리
- **동시성 제어**: 비관적 락을 활용한 동시 입찰 처리
- **성능 최적화**:
    - Querydsl을 활용한 동적 쿼리 작성
    - N+1 문제 해결 (fetch join, @EntityGraph 활용)
- **클라우드 인프라**: S3 Presigned URL 방식으로 클라이언트 직접 업로드 구현 (서버 부하 80% 감소)   
